...
void printHeap(int *heap, int size, int node=0, int d=0) {
    ...
    /*if(size != 0){
        cout<< string(d,'*') << *(heap + node) <<endl; 
    }

    if(leftChild < size){
        printHeap(heap, size, leftChild, ++d);
    }
    ...
    if (rightChild < size){
        printHeap(heap, size, rightChild, size + d);
    }
    if (size != 0){
        cout << setw(d/2) << *(heap + node) << endl;
    }

    if (leftChild < size){
        printHeap(heap, size, leftChild, size + d);
    }
...
void removeByIndex(int* heap, int index, int & size) {
	// TODO: put your code for Question 4 here
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) {
    // TODO: replace the following line with your code for Question 5
