...
Node *& findLargestKey(Node *& a) {
    if (a->right != NULL) {
        return findLargestKey(a->right);
    }
    else {
        return a;
    }
...
bool delete_node(Node*& root, KType key) {
	...
	// case 2: target has two children
        ...
        Node * predecessor_parent = NULL;
        
        target = findLargestKey(currentNode->left);
        predecessor_parent = find_parent(root, target);
        currentNode->key = target->key;
        
        if ( predecessor_parent->left == target )
            predecessor_parent->left = target->left;
        else
            predecessor_parent->right = target->left;
...
int numNodes( Node* root ) {
    ...
    if (root != NULL) {
        size++;
        size = size + numNodes(root->left) + numNodes(root->right);
    }
	return size;
}
...
int height( Node* x ) {
    ...
    if (x == NULL)
        ...
        if (x->left != NULL) {
            left_height = left_height + 1 + height(x->left);
        }
        if (x->right != NULL) {
            right_height = right_height + 1 + height(x->right);
        }
    }
    
    if (left_height > right_height)
        longest_height = left_height;
    else
        longest_height = right_height;
    
	return longest_height;
}
...
int depth( Node* x , Node* root ) {
	 ...
	 */
    int depth_of_x = 0;
    if (find_parent(root, x) == NULL) {
        return 0;
    }
    else {
        depth_of_x = depth_of_x + 1 + depth(find_parent(root, x), root);
    }
	return depth_of_x;
}
